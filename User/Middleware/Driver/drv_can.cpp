...
void TIM_CAN_PeriodElapsedCallback()
{
    ...
    if (mod10 % 3 == 1)
    {
        can_tx_status[1] = CAN_Send_Data(&hcan2, 0x1ff, CAN2_0x1ff_Tx_Data, 8);
    }

    if (mod10 % 3 == 2)
    {
        can_tx_status[2] = CAN_Send_Data(&hcan2, 0x77, CAN2_Gimbal_Tx_Chassis_Data, 8); // 给底盘发送控制命令 按照0x77 ID 发送
    }
