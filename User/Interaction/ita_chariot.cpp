...
#if defined(INFORMATION_PLATFORM)

void Class_Chariot::Information_Platform_Update()
{
    #ifdef CHASSIS
        ...
        Game_Stage = Referee.Get_Game_Stage();
        #endif

        Information_Platform.Set_Robot_ID(Robot_Id);
        Information_Platform.Set_Game_Stage(Game_Stage);
        Information_Platform.Set_Shooter_Barrel_Heat_Limit(Shooter_Barrel_Heat_Limit);
        Information_Platform.Set_Shooter_Barrel_Cooling_Value(Shooter_Barrel_Cooling_Value);
        ...
        //控制类型字节
        #ifdef MINI_PC
        Information_Platform.Set_MiniPC_Status(MiniPC.Get_MiniPC_Status());  
        #endif
        gimbal_pitch = Gimbal.Motor_Pitch_LK6010.Get_True_Angle_Pitch();
        #ifdef chassis
        chassis_velocity_x = Get_Target_Velocity_X();
        chassis_velocity_y = Get_Target_Velocity_Y();
        chassis_omega = Get_Target_Omega();
    #endif

    //设定速度
    tmp_chassis_velocity_x = Math_Float_To_Int(chassis_velocity_x,-1 * Information_Platform.Get_Velocity_X_Max() ,Information_Platform.Get_Velocity_X_Max() ,0,0x7FFF);
    tmp_chassis_velocity_y = Math_Float_To_Int(chassis_velocity_y,-1 * Information_Platform.Get_Velocity_Y_Max() ,Information_Platform.Get_Velocity_Y_Max() ,0,0x7FFF);
    ...
    Information_Platform.Set_Chassis_Velocity_X(tmp_chassis_velocity_x);
    Information_Platform.Set_Chassis_Velocity_Y(tmp_chassis_velocity_y);
    Information_Platform.Set_Chassis_Omega(tmp_chassis_omega);
    Information_Platform.Set_Gimbal_Pitch_Angle(tmp_gimbal_pitch);
...
#if defined(INFORMATION_PLATFORM)  && defined(GIMBAL)
void Class_Chariot::Control_Platform()
{
    //遥控器摇杆值
    ...
    //底盘和云台夹角（弧度制）
        float derta_angle;    
    #if defined(CONTROLLER)  && defined(DR16_REMOTE)
	    /************************************遥控器控制逻辑*********************************************/
        if (Get_DR16_Control_Type()==DR16_Control_Type_REMOTE)
        {
            ...
            //设定矩形到圆形映射进行控制
            gimbal_velocity_x = dr16_l_x * sqrt(1.0f - dr16_l_y * dr16_l_y / 2.0f) * Information_Platform.Get_Velocity_X_Max() ;
            gimbal_velocity_y = dr16_l_y * sqrt(1.0f - dr16_l_x * dr16_l_x / 2.0f) * Information_Platform.Get_Velocity_Y_Max() ;

            //键盘遥控器操作逻辑
            if (DR16.Get_Left_Switch()==DR16_Switch_Status_MIDDLE)  //左中 随动模式
            {
                //底盘随动
                Information_Platform.Set_Chassis_Control_Type(Chassis_Control_Type_FLLOW);   
            }
            if (DR16.Get_Left_Switch()==DR16_Switch_Status_UP)  //左上 小陀螺模式
            {
                Information_Platform.Set_Chassis_Control_Type(Chassis_Control_Type_SPIN);
                chassis_omega = -Information_Platform.Get_Spin_Omega();
    ...
    /************************************键鼠控制逻辑*********************************************/
        ...
        {   
            ...
            if(DR16.Get_Keyboard_Key_Shift() == DR16_Key_Status_PRESSED)  //按住shift加速
            {
                ...
                Information_Platform.Set_Chassis_Control_Type(Sprint_Status_ENABLE);
            }
            else
            {
                DR16_Mouse_Chassis_Shift = 2.0f;
               Information_Platform.Set_Chassis_Control_Type(Sprint_Status_DISABLE); 
            }

            if (DR16.Get_Keyboard_Key_A() == DR16_Key_Status_PRESSED)  //x轴
            {
                gimbal_velocity_x = -Information_Platform.Get_Velocity_X_Max()/DR16_Mouse_Chassis_Shift;
            }
            ...
            {
                gimbal_velocity_y = Information_Platform.Get_Velocity_Y_Max()/DR16_Mouse_Chassis_Shift;
            }
            ...
            {
                if(Information_Platform.Get_Chassis_Control_Type()==Chassis_Control_Type_FLLOW)
                {
                    Information_Platform.Set_Chassis_Control_Type(Chassis_Control_Type_SPIN);
                    chassis_omega = Information_Platform.Get_Spin_Omega();
                }
            ...
            {
                Information_Platform.Set_Referee_UI_Refresh_Status(Referee_UI_Refresh_Status_ENABLE);
            }
            else 
            {
                Information_Platform.Set_Referee_UI_Refresh_Status(Referee_UI_Refresh_Status_DISABLE);
            }
        }
    #endif

    Chassis_Angle = Gimbal.Motor_Yaw.Get_Now_Radian();
    ...
    else if(Information_Platform.Get_Chassis_Control_Type() == Chassis_Control_Type_DISABLE)
    {
        chassis_omega = 0;
    }

    Information_Platform.Set_Target_Velocity_X(chassis_velocity_x);
    Information_Platform.Set_Target_Velocity_Y(chassis_velocity_y);
    Information_Platform.Set_Target_Omega(chassis_omega);
}
#endif
...
void Class_Chariot::Control_Gimbal()
{
    ...
    if(Get_DR16_Control_Type()==DR16_Control_Type_REMOTE)
    {
        ...
        else if(Information_Platform.Get_Chassis_Control_Type() == Chassis_Control_Type_FLLOW &&
    ...
    else if(Information_Platform.Get_Chassis_Control_Type()==Chassis_Control_Type_SPIN)
    {
...
void Class_Chariot::TIM_Calculate_PeriodElapsedCallback()
{
        ...
        Information_Platform_Update();

        Information_Platform.Platform_Tx_Callback();
