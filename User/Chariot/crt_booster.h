 ...
 */
 ...
 float Heat ;
     ...
     void Reload_TIM_Status_PeriodElapsedCallback_new_test();
 ...
 public:
     ...
     inline void Set_Heat_referee(float __Heat_referee);
     void TIM_Calculate_PeriodElapsedCallback();
     void Output();
     void can_shoot(int shoot_count);
     void if_shoot_complete();
    inline void set_heat(int __heat);
 ...
 protected:
     ...
     uint16_t Friction_Torque_Threshold = 1000;
     // 摩擦轮速度判定发弹阈值, 超出则说明已经开机
     float Friction_Omega_Threshold = 600;
 
     // 内部变量
     int heat_per_bullet=10;        // 单发子弹热量
     int cooling_rate_per_sec=10;     // 每秒冷却值
     int max_heat_capacity=50;        // 枪管最大热量
     int current_heat=0;             // 当前热量
     int remain_heat;              // 剩余热量
     int shoot_permission=0;   // 射击许可状态
     int booster_shoot_flag=0;      // 射击状态
     int tmp_shoot_count_target; //目标射击数量
     int Shoot_Count; //当前射击数量
     // 读变量
 
     // 拨弹盘默认速度, 一圈九发子弹, 此速度下与冷却均衡（存疑）
 ...
 void Class_Booster::set_heat(int __heat)
 {
        current_heat = __heat;
 }
